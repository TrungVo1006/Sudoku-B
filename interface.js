...
$(document).keypress(function(evt){
    ...
    else if(evt.which == 68 || evt.which == 100){
      flippage(1);
    }
    else if(evt.which == 65 || evt.which == 97){
      flippage(-1);
    }
...
$(document).on('click', '#helpbutton', function(ev) {
    hidepopups();
    showpopup('#instruction');
    ev.stopPropagation();
});
$(document).on('click', '#sudokusize', function(ev) {
    Sudoku.init(document.getElementById("sudokusize").value);
});
